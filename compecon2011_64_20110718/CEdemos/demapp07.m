...
nplot = [20 20];
[x1,xcoord] = nodeunif(nplot,a,b);

% Compute actual and fitted values on grid
[X, Y] = meshgrid(x1(:,1),y(:,2));
fr = 1./(3-Y.^3+X.^2.*Y);
ycheb = funeval(c,chebproj,x1);                % Chebychev
...
figure(1);
yplot = reshape(fr,nplot(1),nplot(2));
...
error = reshape(ycheb-fr,nplot(1),nplot(2));
