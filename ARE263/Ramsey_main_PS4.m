...
uncertainty_flag=1;
gamma = 0.5;    % Disentangelment constant. 
mu=1; 
sigma = 0.1;
nshocks = 3; 
...
h = figure('name', ['Value function evaluated at k-nodes']);
      set(gca,'fontsize',16)
        ...
        title(sprintf(['Value Function sigma = ' num2str(sigma) ' eta' num2str(eta)]), 'FontSize', 20);
        xlabel('Effective Capital', 'FontSize', 16);
        ylabel('Normalized Value', 'FontSize', 16);
     %saveas(h,['value_function_for_' num2str(num_nodes) '_' interp '_nodes'], 'jpg');
     saveas(h,['u_value_function_for_' num2str(num_nodes) '_' interp '_nodes'], 'epsc');
close(h)

h = figure('name', ['Control rule evaluated at k-nodes']);
      set(gca,'fontsize',16)
        plot(k_nodes_u, consum_u);
        title(sprintf(['Control Rule sigma = ' num2str(sigma) ' eta' num2str(eta)]), 'FontSize', 20);
        xlabel('Effective Capital', 'FontSize', 16);
        ylabel('Effective Consumption', 'FontSize', 16);
     %saveas(h,['control_rule_for_' num2str(num_nodes) '_' interp '_nodes'], 'jpg');
     saveas(h,['u_control_rule_for_' num2str(num_nodes) '_' interp '_nodes'], 'epsc');
close(h)
...
h = figure('name', ['Value function evaluated at k-nodes ']);
     ...
     saveas(h,['u_value_function_dense_'  interp '_' num2str(num_nodes)], 'epsc');
close(h)
...
h = figure('name', ['Value function residual']);
      set(gca,'fontsize',16)
        ...
        title(sprintf(['Value function residual sigma = ' num2str(sigma) ' eta' num2str(eta)]), 'FontSize', 20);
        xlabel('Effective Capital', 'FontSize', 16);
...
h = figure('name', 'Time paths effective');
      set(gca,'fontsize',16)
        ...
        title(sprintf(['Time paths in effective units sigma = ' num2str(sigma) ' eta' num2str(eta)]), 'FontSize', 20);
...
h = figure('name', ['Time paths original units']);
      set(gca,'fontsize',16)
        ...
        title(sprintf(['Time paths in original units sigma = ' num2str(sigma) ' eta' num2str(eta)]), 'FontSize', 20);
        xlabel('Time in years', 'FontSize', 16);
