...
disp(['Solving the model with uncertainty considering ' num2str(nshocks) ' and standard deviation ' num2str(sigma)])  

for i = 1:num_nodes
    index = i;
    
    % maximization step, define the r.h.s. of Bellman as the objective
    val_handle_u = @(cons) Ramsey_bellman_u_PS4(c,cons,k_nodes(index), w, e);
