function [consum,value,c,B,logical_converged,it] = Ramsey_iterate(maxit,tol,damp,k_nodes,value,consum,c)
...
global fspace kappa L options delta consum_min
...
for i = 1:num_nodes 
        ...
        val_handle = @(cons) Ramsey_bellman(c,cons,k_nodes(index));
        ...
        [consum_new,value_new] = fmincon(val_handle,consum(index),[],[],[],[],consum_min,production,[],options);        
...
%% function iteration - update step - calculate new value function approximation
    ...
    if step1>10e+6 && it>1  % Divergence break
        disp(['Breaking function iteration with ',num2str(nodes),' nodes because coefficients changing too much (sign of divergence): ',num2str(step1)])
        break;
    end
