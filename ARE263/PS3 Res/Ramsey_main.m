...
num_nodes = 30; % Number of evaluation nodes & basis functions
...
[consum,value,coeff,B,logical_converged,it] = Ramsey_iterate(maxit,tol,damp,k_nodes,value,consum,coeff);


% Display an error message if the function iteration did not converge
if logical_converged == 0
    disp(['Did not converge. Breaking script.'])
    %break
end
