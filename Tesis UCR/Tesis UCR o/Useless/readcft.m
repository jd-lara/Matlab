...
if nargin<1
  ...
  S.BaseMVA = sscanf(s(1,32:37), '%d');%Lee la base del sistema.  
...
% variables para meter informacion de los branches. 
  ...
  while ~strcmp(s(1:4), '-999'),
      ...
      if (S.Branch.Type(nn,1) == 1) || (S.Branch.Type(nn,1) == 2), %Caso de transformador y transformador con control
          ...
          c=tap^-1;
          S.Branch.Z(nn,1)=  (R+1i*X)*(tap.*exp(1i*alpha));
          S.Branch.B(nn,1) = scanreal(s,41,49);
          S.Branch.YI(nn,1)= (S.Branch.Z(nn,1)^-1)*c(nn,1)*(c(nn,1)-1);
          S.Branch.YJ(nn,1)= (S.Branch.Z(nn,1)^-1)*(1-c(nn,1));
