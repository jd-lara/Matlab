...
bldpfloweqs(S)
L_exact=[]; P=[];
S.Bus.Generation(37:38)=0;
%S.Bus.Load=S.Bus.Load*1.3;
S=pflow(S);
pf=0.95;
s=sin(acos(pf));
%S.Bus.Voltages
[losses,L_k]=losses_exact(S);
x=[S.Machine.MW(1:6)/50;0];
losses_approx2(x,S)
...
lb(2*S.Bus.n+1:end)=0.0*S.Machine.MW(2:6)/(50);
...
ub(2*S.Bus.n+1:end)=S.Machine.MW(2:6)/(50);
